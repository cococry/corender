...
cr_mem_create_gpu_buffer(
    ...
    *o_buf = (struct cr_gpu_buffer_t){0};
    ...
    switch(mem_type) {
        ...
        case CR_GPU_BUFFER_MEM_READBACK:
            usage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT;
            mem_props =
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
        ...
        default:
            CR_FATAL(ctx->log, "Invalid buffer memory type for GPU buffer creation specified: %i.", mem_type);
            break;
    }
    ...
    switch(type) {
        ...
        case CR_GPU_BUFFER_SSBO:
            usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                VK_BUFFER_USAGE_TRANSFER_SRC_BIT; // needed for stats copy-out
            break;

        case CR_GPU_BUFFER_READBACK:
            usage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT;
            break;

        case CR_GPU_BUFFER_NO_TYPE:
            break;

        default:
            CR_FATAL(ctx->log, "Invalid buffer type for GPU buffer creation specified: %i.", type);
            break;
    }
